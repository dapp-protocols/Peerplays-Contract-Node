 ...
 *
 * The collected data is stored in specializations of the @ref Infra::reflector struct template. Member names
 * are accessible via the @ref Infra::member_names::member_name template.
 * 
 * Created by Nathaniel Hourt, with strong inspirations from Daniel Larimer's FC library's static reflection
 */
...
struct field_reflection {
    ...
    static const char* get_name() { return Infra::member_names::member_name<container, index>::value; }
};
...
struct inherited_field_reflection {
    ...
    static const char* get_name() {
        using Reflector = typename Infra::reflector<Base>::native_members::template at<IndexInBase>;
 ...
 *
 *  @brief Specializes Infra::reflector for TYPE where
 ...
 *  @brief Specializes Infra::reflector for TYPE
 *
 *  @param MEMBERS - a sequence of member names.  (field1)(field2)(field3)
 *
 *  @see REFLECT_DERIVED
