 ...
 * 
 * TypeList (properly, TypeList::List) is a utility for handling lists of types at compile time.
 * Created by Nathaniel Hourt, released freely to the world. I hope it helps :)
 */
...
template<typename List, template<typename V, typename R> class Acc, typename I> struct reduce;
template<template<typename V, typename R> class Acc, typename I> struct reduce<List<>, Acc, I> { using type = I; };
template<typename T, typename... Ts, template<typename V, typename R> class Acc, typename I>
struct reduce<List<T, Ts...>, Acc, I> {
    using type = typename reduce<List<Ts...>, Acc, typename Acc<T, I>::type>::type;
};
...
struct transformToKey {
    template<typename> struct transform;
    template<typename Key, typename... Rest>
    struct transform<List<Key, Rest...>> {
        using type = Key;
    };
};
struct transformToValue {
    template<typename> struct transform;
    template<typename Key, typename Value, typename... Rest>
    struct transform<List<Key, Value, Rest...>> {
        using type = Value;
    };
};
...
struct isValid<Map, std::void_t<typename transform<Map, transformToKey>::type,
                                typename transform<Map, transformToValue>::type>> : public std::true_type {};

template<typename Map, typename Key>
constexpr static bool contains = isValid<Map>::value &&
        indexOf<Key, typename transform<Map, transformToKey>::type>::value != -1;

template<typename Map, typename Key, typename Default, typename=void>
struct lookup { using type = Default; };
template<typename Map, typename Key, typename Default>
struct lookup<Map, Key, Default, std::enable_if_t<isValid<Map>::value && contains<Map, Key>>> {
    using Keys = typename transform<Map, transformToKey>::type;
    using Values = typename transform<Map, transformToValue>::type;
    constexpr static int index = indexOf<Key, Keys>::value;
    using type = std::conditional_t<index == -1, Default, typename at<Values, index>::type>;
};
...
using append = concat<L, List<T>>;

/// Prepend a type to a List
template<typename L, typename T>
using prepend = concat<List<T>, L>;
...
/// Transform elements of a TypeList using a Transformer. A Transformer is a type such that the following expression
/// yields the transformed type:
/// typename Transformer::template transform<T>::type
...
/// Evaluate a reduction of the list to a single value by passing values in turn to Accumulator
template<typename L, template<typename Value, typename Reduction> class Accumulator, typename Initial = void>
using reduce = typename impl::reduce<L, Accumulator, Initial>::type;
...
/// Metafunction to determine whether a given type is a TypeList or not
template<typename>
constexpr static bool IsTypeList = false;
template<typename... Ts>
constexpr static bool IsTypeList<List<Ts...>> = true;

/// This namespace contains functionality applying to map-style TypeLists, i.e. TypeLists of two-item TypeLists where
/// the subLists are key-value pairs.
namespace Map {

/// Get a List of keys in the map, i.e., a List containing the first element of every subList
template<typename M>
using Keys = transform<M, impl::map::transformToKey>;

/// Get a List of values in the map, i.e., a List containing the second element of every subList
template<typename M>
using Values = transform<M, impl::map::transformToValue>;

/// Check that the map is well-formed, i.e. it is a List of two-element Lists. Lists of more than two elements are
/// accepted as well.
template<typename M>
constexpr static bool IsValid = impl::map::isValid<M>();

/// Check whether the type Key is in the provided map
template<typename M, typename K>
constexpr static bool Contains = impl::map::contains<M, K>;

/// Get the Value corresponding to the provided Key, or D if no such key is found
template<typename M, typename K, typename D=void>
using Lookup = typename impl::map::lookup<M, K, D>::type;
...
} } } // namespace Infra::TypeList::runtime
